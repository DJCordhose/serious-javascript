...
http.createServer(function(req, res) {
  ...
  if (req.url == '/') {
    ...
    var myAppHtml = React.renderComponentToString(MyApp(props))
      ...
      // do this to provoke warning because of changed state
//  props.items.push(5);
    ...
    res.end(
      ...
      '<script>' +
        ...
        "console.log('component = React.renderComponent(MyApp(" + safeStringify(props) + "), container);'); " +
        // Now that everything has loaded, we can enable the button
        'console.log("component.setProps({disabled: false})");' +
      '</script>'
    )
